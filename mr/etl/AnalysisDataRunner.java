package com.transformer.mr.etl;
 ...
 * hadoop jar transform.jar package_name.AnalysisDataRunner -d
 * 2015-12-20<br/>
 * yarn jar transform.jar package_name.AnalysisDataRunner -d
 * 2015-12-20<br/>
 * 
 *
 */
...
//		//test2(args);
//		//--------------解决本地提交集群运行权限问题-----------------------
//		UserGroupInformation.createRemoteUser("frank").doAs(new PrivilegedAction<Void>() {
//			//使用Void包装类而不是void
//			@Override
//			public Void run() {
//				test2(args);
...
//		});
	...
	public static void test2(final String[] args) {
		UserGroupInformation.createRemoteUser("frank").doAs(new PrivilegedAction<Object>() {
	...
	public void setConf(Configuration that) {
		...
		/* 下面配置是本地提交集群运行 */
		// 添加一些参数
//		this.conf.set("fs.defaultFS", "hdfs://hadoop01.ljf:8020");
//		this.conf.set("mapreduce.framework.name", "yarn");
//		this.conf.set("yarn.resourcemanager.address", "hadoop01.ljf:8032");
//		this.conf.set("mapreduce.app-submission.cross-platform", "true"); // 当开发环境是windows，集群式linux时候，配置为true
		/* 上面配置是本地提交集群运行 结束 */
...
//		((org.apache.hadoop.mapred.JobConf) job.getConfiguration()).setJar("target/transformer-0.0.1.jar");
	...
	private void setJobInputPaths(Job job) throws IOException {
		...
		// hdfs://hadoop01.ljf:8020/eventLogs/2015/12/20
